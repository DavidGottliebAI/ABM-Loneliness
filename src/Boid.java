...
public class Boid {
	 ...
	 * Private parameters:
	 * 
	 * @socialExpected: the amount of **meaningful** social interaction an agent
	 *                  expects to receive
	 * @socialPerceived: the amount of **meaningful** social interaction an agent
	 *                   perceives
	 * @interactionChance: the probability that an agent will interact with another
	 *                     agent
	 * @mutualInterest: a value representing the values and interests of an agent,
...
//	
	 ...
	 * 
	 * @param sensitivity
	 * @params: x, y, mutualInterest, socialExpected, interactionChance,
	 *          sensitivity, fitness, color
	 */
	public Boid(int x, int y, double socialExpected, double[] defaultBehavior, double[] sensitivity, String fitness,
			String color) {
		this.color = color;
		this.position[0] = x;
		this.position[1] = y;
		...
		for (int i = 0; i < sensitivity.length; i++)
			this.sensitivity.add(sensitivity[i]);
		for (int i = 0; i < defaultBehavior.length; i++)
			this.defaultBehavior.add(defaultBehavior[i]);
	}
...
//	}
	...
	/**
	 * perceivedSocialization: adjust an agent's perceived social interaction
	 * positively
	 */
	...
	/**
	 * mutualInterest: calculate if two agents are compatible in respect to common
	 * interest
	 */
