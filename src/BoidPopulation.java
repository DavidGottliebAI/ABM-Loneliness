...
public class BoidPopulation {
	...
	public BoidPopulation(int seed, boolean[] genome, String fitness, int populationSize, int metaPopulationSize, boolean loneliness,
			ArrayList<Hill> hills, ArrayList<int[]> initialPositions, String experimentName) throws IOException {
		...
		for(int i = 0; i < metaPopulationSize ; i++) {
			InitializeBoids boidsInitial = new InitializeBoids("", populationSize, fitness, 1000, this.loneliness, hills, initialPositions, experimentName);
...
//				double[] defaultBehavior = {Dist.calculateDistibutionValue(0.9, 0.05), Dist.calculateDistibutionValue(0, 1), 
//						Dist.calculateDistibutionValue(0.8, 0.2), Dist.calculateDistibutionValue(0.8, 0.1), 
//						Dist.calculateDistibutionValue(0.8, 0.3), Dist.calculateDistibutionValue(0.7, 0.2)};
				double[] defaultBehavior = {1,1,1,1,1,1};
				Boid b = new Boid(boidsInitial.initialPositions.get(j)[0], boidsInitial.initialPositions.get(j)[1], 0.0, defaultBehavior, sensitivity, "one", "black");
//				Boid b = new Boid(random.nextInt(100), random.nextInt(100), 1, sensitivity, "one", "black");
//				Boid b = (j==0) ? new Boid(50, 100, 1, 0, 1, sensitivity, "one", "black") : new Boid(150, 100, 1, 0, 1, sensitivity, "one", "black");
				if(genome[0]) b.setExpected(random.nextDouble());
				if(genome[1]) b.setSensitivity(0, random.nextDouble() * 2 - 1);
				if(genome[2]) b.setSensitivity(1, random.nextDouble() * 2 - 1);
				if(genome[3]) b.setSensitivity(2, random.nextDouble() * 2 - 1);
				if(genome[4]) b.setSensitivity(3, random.nextDouble() * 2 - 1);
				if(genome[5]) b.setSensitivity(4, random.nextDouble() * 2 - 1);
				if(genome[6]) b.setSensitivity(5, random.nextDouble() * 2 - 1);
...
//		allBoids.get(0).writeToCSV(allBoids.get(0).popData, "myfile.csv");
	...
	public InitializeBoids deepCopy(InitializeBoids population, int seed) {
		InitializeBoids newPopulation = new InitializeBoids(population.boids, seed, this.loneliness, population.hills, population.initialPositions, experimentName);
	...
	private void mutate(boolean[] genome, int seed) {
		...
		for(InitializeBoids population : allBoids) {
			for(Boid b : population.boids) {
				...
				if(genome[4] && random.nextInt(100) + 1 <= mutatePercentage) b.setSensitivity(3, dist.calculateDistibutionValue(b.getSensitivity().get(3), 0.1));
				if(genome[5] && random.nextInt(100) + 1 <= mutatePercentage) b.setSensitivity(4, dist.calculateDistibutionValue(b.getSensitivity().get(4), 0.1));
			}
		}
	}

	public void writeToCSV(double[][][][] arr, String filename) throws IOException {
		PrintWriter pw = new PrintWriter(new File("C:\\Users\\gottlijd\\Desktop\\ABML\\src\\" + experimentName + "\\gen_csv\\" + filename));
//		File file = new File(filename);
	...
	public void writeToBoxCSV(double[][] arr, String filename) throws IOException {
		PrintWriter pw = new PrintWriter(new File("C:\\Users\\gottlijd\\Desktop\\ABML\\src\\" + experimentName + "\\box_csv\\" + filename));
		StringBuilder sb = new StringBuilder();
